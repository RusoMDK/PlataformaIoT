...
export default function Sidebar({ onLogout }) {
  ...
  return (
    <aside className="fixed top-[65px] left-0 h-[calc(100vh-56px)] group hover:w-64 w-16 transition-all duration-300 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-r border-gray-200 dark:border-gray-800 shadow-sm flex flex-col">
          ...
          (item, index) =>
            item.divider ? (
              ...
              >
                ...
                <span className="ml-2 truncate opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-sm font-medium">
                  {item.label}
                </span>
              </Link>
            )
      ...
      <div className="p-3">
        <button
          onClick={onLogout}
          className="flex items-center px-3 py-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-800/50 rounded-lg w-full transition"
        >
          <div className="min-w-[24px] flex justify-center items-center">
            <LogOut size={20} />
          </div>
          <span className="ml-2 truncate opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-sm">
